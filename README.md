...
Some time we have to run background/foreground services like location
service in our app. Then we should know, how to handle Flutter
Application LifeCycle. For LifeCycle, we need to implement the abstract
class WidgetsBindingObserver it works when the app goes on foreground
and background.

### WidgetsBindingObserver

WidgetsBindingObserver should be used to get default behaviors for all
of the handlers. In our case, when we want to listen to the
AppLifecycleState and call stop/start on our services.
...
- `dispose()`: is called when the State object is removed, which is permanent. This method is where we should unsubscribe and
  cancel all animations, streams, etc
